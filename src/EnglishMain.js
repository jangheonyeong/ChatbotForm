...
const listContainer = document.getElementById("englishList");

async function loadEnglishChatbots() {
  const chatbotRef = collection(db, "chatbots");
  const q = query(chatbotRef, where("subject", "==", "영어"));
  ...
  querySnapshot.forEach((doc) => {
    ...
    // ✅ 예시 처리 (문자열 배열, 객체 배열, 키-값 객체 모두 지원)
